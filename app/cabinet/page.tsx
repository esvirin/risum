    ...
    <div className="space-y-8">
      <header className="space-y-2">
        <h1 className="text-3xl font-bold tracking-tight">
          Hello, {customer.name.first} {customer.name.last}
        </h1>
        <p className="text-muted-foreground">
          Welcome to your personal dashboard {customer.email}
        </p>
      </header>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="flex items-center justify-between mt-4">
              <Badge
                variant={member.role === "active" ? "default" : "destructive"}
                className={
                  member.role === "active"
                    ? "bg-primary/20 text-primary hover:bg-primary/30 shadow-none"
                    : ""
                }
              >
              ...
              >
                <div>
                  ...
                  <div className="text-sm text-muted-foreground mt-1">
                    {new Date(cls.start).toLocaleDateString()} at{" "}
                    {new Date(cls.end).toLocaleTimeString([], {
                      hour: "2-digit",
                      minute: "2-digit",
                    })}
                  </div>
